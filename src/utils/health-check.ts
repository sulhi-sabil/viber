...
export function formatAggregateHealthResult(
  ...
  const lines: string[] = [
    ...
    `╠${"═".repeat(BOX_WIDTH)}╣`,
    `║  ${padLine(`${statusIcon} Overall Status: ${result.status.toUpperCase()}`)}║`,
    `║  ${padLine(`Checked at: ${new Date(result.timestamp).toLocaleString()}`)}║`,
    `║${" ".repeat(BOX_WIDTH)}║`,
    `║  ${padLine("📊 Summary")}║`,
    `║     ${padLine(`Total Services: ${result.summary.total}  •  ✅ Healthy: ${result.summary.healthy}`)}║`,
