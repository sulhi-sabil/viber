  ...
  describe("execute", () => {
    ...
    it("should reject invalid UUID formats", async () => {
      ...
      for (const key of invalidKeys) {
        await expect(manager.execute(key, operation)).rejects.toThrow(
    ...
    it("should handle errors in operation", async () => {
      ...
      await expect(manager.execute(idempotencyKey, operation)).rejects.toThrow(
        "Operation failed",
      );
    ...
    it("should handle concurrent requests (without locking)", async () => {
      ...
      expect(results.every((r) => r.idempotencyKey === idempotencyKey)).toBe(
        true,
      );
    });
  });
