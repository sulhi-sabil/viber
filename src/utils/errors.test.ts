...
describe('mapHttpStatusCodeToErrorCode', () => {
  ...
  it('should map 429 to RATE_LIMIT_EXCEEDED', () => {
    expect(mapHttpStatusCodeToErrorCode(429)).toBe(ErrorCode.RATE_LIMIT_EXCEEDED);
  });
...
describe('wrapError', () => {
  ...
  it('should wrap Error with custom message and error code (message path)', () => {
    const error = new Error('Custom error');
    const wrapped = wrapError(error, 'Failed to process', ErrorCode.VALIDATION_ERROR);
...
describe('Edge Cases', () => {
  ...
  it('should handle wrapping error with custom error code', () => {
    const error = new Error('Custom error');
    const wrapped = wrapError(error, 'Custom message', ErrorCode.VALIDATION_ERROR);
  ...
  it('should map 503 to SERVICE_UNAVAILABLE', () => {
    expect(mapHttpStatusCodeToErrorCode(503)).toBe(ErrorCode.SERVICE_UNAVAILABLE);
  });
