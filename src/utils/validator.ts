...
export class Validator {
  ...
  static integer(value: unknown, fieldName: string = 'field'): void {
    ...
    if (!Number.isInteger(value as number)) {
      throw new ValidationError(`${fieldName} must be an integer, received: ${value}`);
    }
  }

  static boolean(value: unknown, fieldName: string = 'field'): void {
    if (typeof value !== 'boolean') {
      throw new ValidationError(`${fieldName} must be a boolean, received: ${typeof value}`);
    }
  }
  ...
  static array(value: unknown, fieldName: string = 'field'): void {
    if (!Array.isArray(value)) {
      ...
      throw new ValidationError(`${fieldName} must be an array, received: ${actualType}`);
    }
  }
  ...
  static minLength(value: unknown, min: number, fieldName: string = 'field'): void {
    this.string(value, fieldName);
  ...
  static maxLength(value: unknown, max: number, fieldName: string = 'field'): void {
    this.string(value, fieldName);
    const strValue = value as string;
  ...
  static min(value: unknown, min: number, fieldName: string = 'field'): void {
    ...
    if (numValue < min) {
      throw new ValidationError(`${fieldName} must be at least ${min} (got: ${numValue})`);
    }
  }
  ...
  static max(value: unknown, max: number, fieldName: string = 'field'): void {
    ...
    const numValue = value as number;
    if (numValue > max) {
      throw new ValidationError(`${fieldName} must be no more than ${max} (got: ${numValue})`);
    }
  }
  ...
  static pattern(value: unknown, regex: RegExp, fieldName: string = 'field'): void {
    this.string(value, fieldName);
    if (!regex.test(value as string)) {
      throw new ValidationError(`${fieldName} does not match required pattern: ${regex.source}`);
    }
  }
...
export class SchemaValidator<T extends Record<string, unknown>> {
  ...
  addField<K extends keyof T>(field: K, ...rules: ValidationRule[]): SchemaValidator<T> {
...
export function sanitizeInput(input: unknown, escapeHtml: boolean = true): unknown {
